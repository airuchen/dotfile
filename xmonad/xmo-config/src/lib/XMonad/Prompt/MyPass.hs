...
-- | A prompt to retrieve a 'user' field from a given entry
--
passUserPrompt :: XPConfig -> X()
passUserPrompt = mkPassPrompt "User for entry" selectUser

-- | A prompt to retrieve a 'url field from a given entry
--
passOpenUrlPrompt :: XPConfig -> X()
passOpenUrlPrompt = mkPassPrompt "Open URL for entry" openUrl
...
-- | Get a field from a secret
--
getSecret :: String -> String -> String
getSecret fieldName command = "awk  '/^" ++ fieldName ++ ":/{if (NR>1) {printf $2; exit}}' | " ++ command

-- | Select a username.
--
selectUser :: String -> X ()
selectUser passLabel = spawn $ "pass show \"" ++ escapeQuote passLabel ++ "\" | " ++ getSecret "user" "xclip -selection clipboard"

-- | Select a url.
--
openUrl :: String -> X ()
openUrl passLabel = spawn $ "pass show \"" ++ escapeQuote passLabel ++ "\" | " ++ getSecret "url" "xargs xdg-open"
