...
ls.add_snippets("cmake", {
  ...
  s({ trig = "catkin_package", condition = line_begin },
    { t({ "catkin_package(", "  INCLUDE_DIRS ", "    " }), i(1, "include"), t({ "", "  LIBRARIES ", "    " }), i(2,
      "${PROJECT_NAME}"), t({ "", "  CATKIN_DEPENDS", "    " }), i(3, "${dependencies}"), t({ "", ")" }) }),
  s({ trig = "adt", condition = line_begin },
    { t("ament_target_dependencies("), i(1, "${PROJECT_NAME}"), t(" "), i(2, "${dependencies}"), t(" "), i(3,
      "${${PROJECT_NAME}_EXPORTED_TARGETS}"), t(")") }),
  ...
  s({ trig = "ib", condition = line_begin }, {
    t("install(TARGETS "), i(1, "${PROJECT_NAME}"),
    t({ "",
